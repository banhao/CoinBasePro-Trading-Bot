...
-  quote_lower_limit - 设定锚定货币最低资金限额，当达到该限额后买入行为将会停止，资金按照换算成USDC计算。假如你有相当于1000USDC的比特币在账户里，当设定限额为500时，一旦卖出比特币买入其他加密货币后，比特币的余额小于500USDC时，将不会再有任何卖出比特币买入其他加密货币的交易行为。此参数的目的是为了当有需要保留一部分锚定货币做长期投资打算，而不打算用来做货币对交易时使用。
